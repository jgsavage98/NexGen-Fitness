...
export class DatabaseStorage implements IStorage {
  ...
  async getTrainerClientUnreadCount(clientId: string): Promise<number> {
    // Count unread messages FROM the client TO the trainer that need trainer attention
    // This excludes AI messages generated BY the trainer
    const result = await db
      ...
      .where(
        and(
          ...
          eq(chatMessages.chatType, 'individual'),
          eq(chatMessages.isAI, false), // Only human messages, not AI messages
          eq(chatMessages.isRead, false),
          eq(chatMessages.status, 'approved')
        )
      );
    ...
    console.log(`Trainer unread count for client ${clientId}: ${count} (client messages only)`);
    return count;
  }
