...
<body>
  ...
  app.get('/api/trainer/clients', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Add unread count property for each client (messages from client that trainer needs to see)
      const clientsWithCount = await Promise.all(clients.map(async (client) => {
        // Count unread messages FROM client TO trainer (excluding AI messages generated by trainer)
        const unreadCount = await storage.getTrainerClientUnreadCount(client.id);
