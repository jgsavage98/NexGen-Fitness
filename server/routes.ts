...
<body>
  ...
  app.put('/api/trainer/ai-settings', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const settings = req.body;
      await storage.saveAISettings(userId, settings);
  ...
  // Auto topic generation endpoint
  app.post('/api/trainer/generate-topic', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const settings = await storage.getAISettings(userId);
      const topic = await generateAutoTopic(settings);
      
      if (!topic) {
        return res.status(500).json({ message: "Failed to generate topic" });
      }
      
      // Optionally save the generated topic as a group chat message
      if (req.body.postToGroup) {
        const savedMessage = await storage.saveGroupChatMessage({
          userId: 'coach_chassidy',
          message: topic,
          isAI: true,
          metadata: { isAutoGenerated: true, topicType: 'manual' }
        });
        ...
        res.json({ topic, posted: true, message: savedMessage });
      } else {
        res.json({ topic, posted: false });
      }
    } catch (error) {
